...
export default function Chatroom() {
  ...
  const socket = io("http://192.168.29.66:8000");
  socket.on("connect", () => {
  ...
  const deleteMessages = async (messageIds) => {
    ...
    try {
      const response = await axios.post("http://192.168.29.66:4000/delete", {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const response = await axios.get("http://192.168.29.66:4000/messages", {
