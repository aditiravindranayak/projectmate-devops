...
export default function Bio() {
    ...
    const fetchUserDescription = async () => {
        try {
            const response = await axios.get(`http://192.168.29.66:4000/users/${userId}`);
    ...
    const updateUserDescription = async () => {
        try {
            const response = await axios.put(
                `http://192.168.29.66:4000/users/${userId}/description`,
    ...
    const handleAddImage = async () => {
        try {
            const response = await axios.post(`http://192.168.29.66:4000/users/${userId}/project-images`, {
                imageUrl: imageUrl
            });
...
const addLookingFor= async (lookingFor)=>{
  try {
    const response = await axios.put(
      `http://192.168.29.66:4000/users/${userId}/looking-for`,
      {
...
const removeLookingFor = async (lookingFor) => {
  try {
    const response = await axios.put(
      `http://192.168.29.66:4000/users/${userId}/looking-for/remove`,
      {
        lookingFor: lookingFor,
      }
    );
...
const addKeywords= async (keywords)=>{
  try {
    const response = await axios.put(
      `http://192.168.29.66:4000/users/${userId}/keywords/add`,
      {
        keywords: keywords,
      }
    );
...
try {
  const response = await axios.put(
    `http://192.168.29.66:4000/users/${userId}/keywords/remove`,
    {
      keywords: keywords,
    }
  );
